	...
	public PluginResult execute(String action, JSONArray args, String callbackId) {
		...
		if (action.equals("initGA")) {
			try {
				...
				//return true;
				return new PluginResult(PluginResult.Status.OK, "initGA - id = " + args.getString(0) + "; interval = " + args.getInt(1) + " seconds");
			...
			try {
				...
				//return true;
				return new PluginResult(PluginResult.Status.OK,"trackEvent - category = " + args.getString(0) + "; action = " + args.getString(1) + "; label = " + args.getString(2) + "; value = " + args.getInt(3));
			} catch (final Exception e) {
				//callback.error(e.getMessage());
			}
		} else if (action.equals("trackPage")) {
			Log.v("GAPlugin","trackPage");
			try {
				tracker.sendView(args.getString(0));
				Log.v("GAPlugin","trackPage: " + args.getString(0));
				//callback.success("trackPage - url = " + args.getString(0));
				//return true;
				return new PluginResult(PluginResult.Status.OK,"trackPage - url = " + args.getString(0));
			} catch (final Exception e) {
				Log.v("GAPlugin","trackPage error");
				//callback.error(e.getMessage());
				return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
			}
			...
			try {
				...
				return new PluginResult(PluginResult.Status.OK,"setVariable passed - index = " + args.getInt(0) + "; value = " + args.getString(1));
				//return true;
			} catch (final Exception e) {
				//callback.error(e.getMessage());
				return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
			...
			try {
				...
				//return true;
				return new PluginResult(PluginResult.Status.OK, "setDimension passed - index = " + args.getInt(0) + "; value = " + args.getString(1));
			} catch (final Exception e) {
				//callback.error(e.getMessage());
				return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
			}
